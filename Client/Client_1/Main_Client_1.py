...
def key_init():
    ...
    print(KeyFile_Server2.big_key_original[:50])
    print(KeyFile_Server2.big_nonce_original[:50])
    KeyFile_Server2.key_choice()
    KeyFile_Server1.big_key_nonce_generator()
    print(KeyFile_Server1.big_key_original[:50])
    print(KeyFile_Server1.big_nonce_original[:50])
    ...
    while len(big_key_nonce) == 0:
        ...
        bigkeynonce_sum1 = File_Manipulation.SHA512_checksum_creation(big_key_nonce[0].encode())
        print("Somme du fichier : ", bigkeynonce_sum1)
        encrypted_key1, tag1, nonce1 = DH_Algorithm_Server1.encrypt(KeyFile_Server1.big_key_nonce_format(1, bigkeynonce_sum1, big_key_nonce[0]))
        print("tag créé : ", tag1)
        print("nonce créé : ", nonce1)
        Server1_conn.sending(KeyFile_Server1.big_key_nonce_format(2, tag1, nonce1, encrypted_key1), 1)
    ...
    while len(big_key_nonce) == 1:
        ...
        bigkeynonce_sum2 = File_Manipulation.SHA512_checksum_creation(big_key_nonce[1].encode())
        print("Somme du fichier : ", bigkeynonce_sum2)
        encrypted_key2, tag2, nonce2 = DH_Algorithm_Server2.encrypt(KeyFile_Server2.big_key_nonce_format(1, bigkeynonce_sum2, big_key_nonce[1]))
        print("tag créé : ", tag2)
        print("nonce créé : ", nonce2)
        Server2_conn.sending(KeyFile_Server2.big_key_nonce_format(2, tag2, nonce2, encrypted_key2), 1)
...
def key_init():
    ...
    while True:
        if len(big_key_nonce) == 0:
            ...
            big_key_nonce.append(KeyFile_Mine.big_key_nonce_format(0))
            part1, part2 = File_Manipulation.split_file(big_key_nonce[2])
            part1_sum = File_Manipulation.SHA512_checksum_creation(part1.encode())
            encrypted_key, tag, nonce = DH_Algorithm_Client.encrypt((KeyFile_Server1.big_key_nonce_format(1, part1_sum, part1)))
            Server1_conn.sending(KeyFile_Client.big_key_nonce_format(2, tag, nonce, encrypted_key), 1)
            part2_sum = File_Manipulation.SHA512_checksum_creation(part2.encode())
